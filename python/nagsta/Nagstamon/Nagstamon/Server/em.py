...
class EmServer(GenericServer):
    ...
    def open_nagios(self):
        webbrowser.open(self.em_url + "/Monitor.action")
        # debug
        if str(self.conf.debug_mode) == "True":
            self.Debug(server=self.get_name(),
            debug="Open monitor web page " + self.em_url + "/Monitor.action")

    def open_services(self):
        webbrowser.open(self.em_url + "/reports/detailed")
        # debug
        if str(self.conf.debug_mode) == "True":
            self.Debug(server=self.get_name(),
    ...
    def open_tree_view(self, host, service=""):
        ...
        if str(self.conf.debug_mode) == "True":
            ...
            host)
        webbrowser.open(self.em_url + '/reports/detailed/show/' + host)
...
#                if c["status"] != 'up':
#### COMENTED PREV LINE JUST FORCE PINGDOM TO THROW AN ALERT  AND GET TO WORK WITH THE MENUS ...  REMEMBER TO ENABLE IT AND THE IFS BELOW AS WELL 
                if c["status"] == 'up':
		    status = 'CRITICAL'
